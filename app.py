...
with st.sidebar:
    ...
    if st.button("Initialize Hybrid Sync"):
        if uploaded_files:
            ...
            st.success(f"Hybrid Sync: {time.time()-start_time:.2f}s")
...
if st.session_state.data_frames:
    with st.expander("📊 Data Visualizer"):
        ...
        else: fig = px.scatter(df, x=x_axis, y=y_axis)
    ...
    with st.chat_message("assistant"):
        if not st.session_state.vector_db:
            ...
            try:
                ...
                # Combine Semantic (0.7 weight) and Keyword (0.3 weight)
                ensemble_retriever = EnsembleRetriever(retrievers=[faiss_retriever, bm25_retriever], weights=[0.7, 0.3])

                prompt_template = ChatPromptTemplate.from_messages([
                    ("system", "You are the Nexus Technical Agent. Answer using context. Mention source files.\n\nContext:\n{context}"),
                ...
                st.caption(f"⏱️ Analysis: {time.time()-analysis_start:.2f}s | Mode: {mode} (Hybrid Search)")
